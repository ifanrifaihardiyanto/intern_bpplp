...
<script>
    ...
    function getData(date, area) {
        ...
        $(document).ready(() => {
            $.ajax({
                type: "POST",
                url: '<?php echo base_url() . "index.php/program_bintang/program_bintang/ajax_post_get_data_summary" ?>',
                data: {
                    "month": date,
                ...
                success: (response) => {
                    ...
                    // console.log(response.filter.area);
                    ...
                    // Header table
                    let keys_header = Object.keys(response.header);
                    $.each(response.header, (witel, dataItems) => {
                        // console.log(dataItems);
                        textHtmlHead += `<th>${dataItems}</th>`
                    });
                    textHtmlHead += `<th>TOTAL ACH</th>`
                    textHtmlHead += `</tr>`
                    ...
                    if (response.data && keys_data.length !== 0) {
                        $.each(response.data, (witel, dataItems) => {
                            ...
                            if (response.filter.area == 'WITEL') {
                                ...
                                textHtml += `<td>${dataItems.ach_0}%</td>`
                                textHtml += `<td>${dataItems.ach_1}%</td>`
                                textHtml += `<td>${dataItems.ach_2}%</td>`
                                textHtml += `<td>${dataItems.ach_3}%</td>`
                                textHtml += `<td>${dataItems.ach_4}%</td>`
                                textHtml += `<td>${dataItems.ach_5}%</td>`
                                textHtml += `<td>${dataItems.ach_6}%</td>`
                                textHtml += `<td>${dataItems.ach_7}%</td>`
                            }
                            ...
                            textHtml += `<td>${dataItems.total_ach_all}%</td>`
                            textHtml += `</tr>`
                        });
                    } else {
                        textHtml += `<tr>
